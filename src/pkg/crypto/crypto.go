...
func Encrypt(privateKey string, publicKey string, textToEncrypt string) (string, error) {
	...
	cipherBytes, err := aesEncrypt([]byte(textToEncrypt), secretKey)
	if err != nil {
...
func main() {
	...
	case "encrypt":
		privateKey := args[1]
		publicKey := args[2]
		cipherText, err := Encrypt(privateKey, publicKey, "Ondc is great initiative.")
		if err == nil {
